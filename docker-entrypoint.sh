...
rm -f /var/run/munge/munge.socket.2 /var/run/munge/munge.socket.2.lock
...
if [ "$1" = "slurmdbd" ]; then
    ...
    {
        # Source config if necessary
        . /etc/slurm/slurmdbd.conf
        
        until echo "SELECT 1;" | mysql --host="$StorageHost" --user="$StorageUser" --password="$StoragePass" --silent --skip-column-names &>/dev/null; do
