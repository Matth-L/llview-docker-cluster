...
//    console.log("gen_project_user_link: "+config+" "+project+" "+user);
  let result = "";
  let url = 'index.html?config='+config+(self.helperContext.is_demo?"&demo":"")+'&project='+project+'&user='+user;
  if (self.helperContext.permission && ["observer","support","project"].indexOf(self.helperContext.permission) != -1) {
...
//	console.log("gen_user_projects_link: "+config+" "+user+" "+project);
  let url = 'index.html?config='+config+(self.helperContext.is_demo?"&demo":"")+'&project='+project+'&user='+user;
  if (self.helperContext.permission && ["observer","support"].indexOf(self.helperContext.permission) != -1) {
    if (result.length > 0) {
      result += ", ";
    }
...
//	console.log("gen_projects_link: "+config+" "+project);
  let url = 'index.html?config='+config+(self.helperContext.is_demo?"&demo":"")+'&project='+project;
  if (self.helperContext.permission && ["observer","support","project"].indexOf(self.helperContext.permission) != -1) {
     if (result.length > 0) {
      result += ", ";
    }
...
//    console.log("gen_project_link: "+config+" "+project);
  let result = "";
  let url = 'index.html?config='+config+(self.helperContext.is_demo?"&demo":"")+'&project='+project;
  if (self.helperContext.permission && ["observer","support","mentor"].indexOf(self.helperContext.permission) != -1) {
    result = '<a href="'+url+'">'+project+'</a>';
  } else {
    result = project;
  }
...
//    console.log("gen_mentor_link: "+config+" "+mentor);
  let result = "";
  let url = 'index.html?config='+config+(self.helperContext.is_demo?"&demo":"")+'&mentor='+mentor;
  if (self.helperContext.permission && ["observer","support"].indexOf(self.helperContext.permission) != -1) {
...
Handlebars.registerHelper('gen_search_jobid_link', function(jobid) {
  ...
  let result = "";
  if (jobid in self.helperContext.mapjobid_to_day && self.helperContext.permission && ["observer","support"].indexOf(self.helperContext.permission) != -1) {
...
Handlebars.registerHelper('gen_jobid_list_link', function(jobidlist) {
  ...
  jobids.forEach((jobidstatus) => {
    ...
    jobstatus = matches[2] ?? "";
    newline = matches[3] ?? "";
    if (matches[1] in self.helperContext.mapjobid_to_day) {
...
Handlebars.registerHelper('user_link_jobr', function(project,user,config_prefix,system) {
  ...
  let config = ((!config_prefix || typeof config_prefix != "string")?self.helperContext.system:config_prefix)+'_user';
  config += (!system || typeof system != "string")?"":"_system";
  let result = "";
  let url = 'index.html?config='+config+(self.helperContext.is_demo?"&demo":"")+'&project='+project+'&user='+user;
  url += (!system || typeof system != "string")?'':'&system='+system;
  if (self.helperContext.permission && ["observer","support","project","advisor","mentor"].indexOf(self.helperContext.permission) != -1) {
...
Handlebars.registerHelper('mentor_link', function(user,config_prefix) {
  let config = ((!config_prefix || typeof config_prefix != "string")?self.helperContext.system:config_prefix)+'_mentor';
  let result = "";
  let url = 'index.html?config='+config+(self.helperContext.is_demo?"&demo":"")+'&user='+user;
  if (self.helperContext.permission && ["observer","support"].indexOf(self.helperContext.permission) != -1) {
    result = '<a href="'+url+'">'+ user +'</a>';
  } else {
    result = user;
...
Handlebars.registerHelper('project_link', function(project,config_prefix,system) {
  let config = ((!config_prefix || typeof config_prefix != "string")?self.helperContext.system:config_prefix)+'_project';
  config += (!system || typeof system != "string")?"":"_system";
  let result = "";
  let url = 'index.html?config='+config+(self.helperContext.is_demo?"&demo":"")+'&project='+project;
  url += (!system || typeof system != "string")?'':'&system='+system;
  if (self.helperContext.permission && ["observer","support","advisor","mentor","project"].indexOf(self.helperContext.permission) != -1) {
...
Handlebars.registerHelper('projectbudget_link', function(project,budget,config_prefix) {
  let config = ((!config_prefix || typeof config_prefix != "string")?self.helperContext.system:config_prefix)+'_budget';
  let result = "";
  let url = 'index.html?config='+config+(self.helperContext.is_demo?"&demo":"")+'&project='+project+'&budget='+budget;
  if (self.helperContext.permission && ["observer","support","advisor","mentor","project"].indexOf(self.helperContext.permission) != -1) {
...
Handlebars.registerHelper('supportquota_link', function(project,budget,system,config_prefix) {
  let config = ((!config_prefix || typeof config_prefix != "string")?self.helperContext.system:config_prefix)+'_support_quota';
  if (self.helperContext.permission && ["observer"].indexOf(self.helperContext.permission) != -1) {
    config = ((!config_prefix || typeof config_prefix != "string")?self.helperContext.system:config_prefix)+'_observer_quota';
  }
  let result = "";
  let url = 'index.html?config='+config+(self.helperContext.is_demo?"&demo":"")+'&project='+project+'&budget='+budget+'&system='+system;
  if (self.helperContext.permission && ["observer","support","advisor","mentor","project","pipa"].indexOf(self.helperContext.permission) != -1) {
...
Handlebars.registerHelper('projectquota_link', function(project,budget,system,config_prefix) {
  let config = ((!config_prefix || typeof config_prefix != "string")?self.helperContext.system:config_prefix)+'_quota';
  let result = "";
  let url = 'index.html?config='+config+(self.helperContext.is_demo?"&demo":"")+'&project='+project+'&budget='+budget+'&system='+system;
  if (self.helperContext.permission && ["observer","support","mentor","pipa"].indexOf(self.helperContext.permission) != -1) {
    result = '<a href="'+url+'">'+system+'</a>';
  } else {
    result = system;
  }
...
Handlebars.registerHelper('useronly_link', function(user,config_prefix) {
  ...
  let result = "";
  let url = 'index.html?config='+config+(self.helperContext.is_demo?"&demo":"")+'&user='+user;
  if (self.helperContext.permission && ["observer","support","advisor","mentor","project","user"].indexOf(self.helperContext.permission) != -1) {
    result = '<a href="'+url+'">'+user+'</a>';
  } else {
    result = user;
  }
...
Handlebars.registerHelper('pipaonly_link', function(pipa,config_prefix) {
  let config = ((!config_prefix || typeof config_prefix != "string")?self.helperContext.system:config_prefix)+'_pipa';
  let result = "";
  let url = 'index.html?config='+config+(self.helperContext.is_demo?"&demo":"")+'&pi_pa='+pipa;
  if (self.helperContext.permission && ["observer","support","advisor","mentor","pipa"].indexOf(self.helperContext.permission) != -1) {
...
Handlebars.registerHelper('mentoronly_link', function(mentors,config_prefix) {
  ...
  mentors.forEach(function(mentor) {
    ...
    if (self.helperContext.permission && ["observer","support","advisor","mentor"].indexOf(self.helperContext.permission) != -1) {
      result += '<a href="'+url+'">'+mentor+'</a>';
    } else {
      result += mentor;
    }
...
Handlebars.registerHelper('tagonly_link', function(tag,config_prefix) {
  let config = ((!config_prefix || typeof config_prefix != "string")?self.helperContext.system:config_prefix)+'_inspector';
  let result = "";
  let url = 'index.html?config='+config+(self.helperContext.is_demo?"&demo":"")+'&tag='+tag;
  if (self.helperContext.permission && ["observer","support","advisor","mentor"].indexOf(self.helperContext.permission) != -1) {
...
Handlebars.registerHelper('user_link', function(project,budget,login,config_prefix) {
  ...
  let result = "";
  let url = 'index.html?config='+config+(self.helperContext.is_demo?"&demo":"")+'&project='+project+'&budget='+budget+'&user='+login;
  if (self.helperContext.permission && ["observer","support","advisor","mentor","project","user"].indexOf(self.helperContext.permission) != -1) {
...
Handlebars.registerHelper('dataproject_link', function(project,config_prefix,fs) {
  let config = ((!config_prefix || typeof config_prefix != "string")?self.helperContext.fs:config_prefix)+'_dataproject';
...
Handlebars.registerHelper('mentorsnapshot_link', function(spec,mentor,config_prefix) {
  let config = ((!config_prefix || typeof config_prefix != "string")?self.helperContext.system:config_prefix)+'_mentor';
  let result = "";
  let url = './snapshots/'+spec+'/index.html?config='+config+(self.helperContext.is_demo?"&demo":"")+'&mentor='+mentor;
  if (self.helperContext.permission && ["observer","support","advisor","mentor"].indexOf(self.helperContext.permission) != -1) {
...
Handlebars.registerHelper('supportsnapshot_link', function(spec,config_prefix) {
  let config = ((!config_prefix || typeof config_prefix != "string")?self.helperContext.system:config_prefix)+'_support';
  let result = "";
  let url = './snapshots/'+spec+'/index.html?config='+config+(self.helperContext.is_demo?"&demo":"");
  if (self.helperContext.permission && ["observer","support"].indexOf(self.helperContext.permission) != -1) {
    result = '<a href="'+url+'">'+spec+'</a>';
  } else {
    result = spec;
...
Handlebars.registerHelper('cell_color', function(value,defscale) {
  ...
  if (typeof defscale !== "string") {
    ...
    colorscale_name = self.helperContext.colorscale ?? self.helperContext.default_colorscale;
  } else {
    if (value <= 0.00005) {
  ...
  if(self.helperContext.used_colorscales && colorscale_name in self.helperContext.used_colorscales) {
    color_wheel = self.helperContext.used_colorscales[colorscale_name];
  } else {
    function ramp(name,n) {
      ...
      */
      ...
      if (d3[`scheme${name}`] && d3[`scheme${name}`][n]) {
        ...
        colors = Array.from({ length: n }, (_, i) => d3.rgb(interpolate(i / (n - 1))).hex());
      }
      return colors
    }
    ...
    self.helperContext.used_colorscales[colorscale_name] = color_wheel;
    
    // Send a command message back to the main thread to update its cache
    // with the new generated colorscale 'colorscale_name' given by 'color_wheel'
    self.postMessage({
      command: 'cache_colorscale',
      name: colorscale_name,
      scale: color_wheel
    });
  }

  // If no colorscale is selected, return empty style.
  if (!color_wheel) {
    return "";
  }

  // Send a command message back to the main thread to add the colorscale controls
  if (typeof defscale !== "string" && !self.addControlsCalled) {
    self.postMessage({ command: 'add_controls' });
    // Controls are already added, no need to call it again
    // so we set the variable below to avoid that.
    self.addControlsCalled = true;
  }

  let ind = Math.round(value * (color_wheel.length - 1));
  let dark = d3.lab(color_wheel[ind]).l < 50;
  return `background-color: ${color_wheel[ind]} !important; color: ${dark ? '#fff' : '#000'} !important;`;
});

Handlebars.registerHelper('mreporttemplate_link', function(filename,mentor) {
  if (self.helperContext.permission && ["observer","support","project","advisor","mentor"].indexOf(self.helperContext.permission) != -1) {
    result = '<a href="data/'+(self.helperContext.is_demo?'DEMO/':'')+'mentor/'+mentor+'/reports/'+filename+'" type="text/plain" download>'+filename+'</a>';
  } else {
    result = 'TXT';
  }
  return new Handlebars.SafeString(result);
...
//    console.log("pdf_link: "+grp+" "+user+" "+filename);
  ...
  if ( (filename) && (filename!='-') ) {
    let url = 'data/'+(self.helperContext.is_demo?'DEMO/':'')+"projects/"+grp+'/'+user+'/'+((typeof filename !== 'undefined')?filename:'#');
